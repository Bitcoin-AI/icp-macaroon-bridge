/* eslint-disable */
// Modfied from https://lightningdecoder.com/
...
const bitcoinjsAddress = bitcoinjs.address;
...
function encode (inputData, addDefaults) {
  ...
  if (canReconstruct) {
    ...
    gave one. */
    if (nodePublicKey) {
      let recoveredPubkey = secp256k1.ecdsaRecover(Buffer.from(data.signature, 'hex'), data.recoveryFlag,payReqHash, true)
...
function decode (paymentRequest, network) {
  ...
  let payReqHash = sha256(toSign)
  let sigPubkey = secp256k1.ecdsaRecover(sigBuffer, recoveryFlag,payReqHash, true)
